...
def load_crop_images(img_dir, vid, start_frame, lengths):
    ...
    if lengths<=128*16:
        ...
        for start in range(0, lengths, 16):
            ...
            if end - start < 16:
                start = max(0, end - 16)
            stride_list.append(img_frames[start:end, :, :, :])
        print("—————————————img_loaded!———————————————")
...
for vid in os.listdir("/home/gwenbo/data/TACoS_image"):
    ...
    if os.path.exists("/home/gwenbo/local_adv/data/"+vid+".npy"):
        continue
    ...
    np.save(os.path.join("/home/gwenbo/local_adv/data",vid),arr=imgs)
    del imgs
    # del stride_list